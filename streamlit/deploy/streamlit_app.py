...
page = st.sidebar.radio("Navigation", ["Overview", "Equipment Health", "Anomaly Alerts", "ML Predictions (Snowflake ML)", "Maintenance Schedule", "AI Work Order", "Search Maintenance Docs", "Ask Maintenance"], label_visibility="collapsed")
st.sidebar.divider()
...
if page == "Overview":
...
elif page == "AI Work Order":
    st.title("AI Work Order Generator")
    st.caption("Cortex AI generates prioritized work orders from sensor anomalies and equipment health")
    try:
        eq = coerce_numeric(session.sql("SELECT EQUIPMENT_ID, NAME, STATUS, CRITICALITY, HEALTH_SCORE FROM MANUFACTURING_MAINTENANCE.CURATED.EQUIPMENT_HEALTH WHERE STATUS IN ('CRITICAL','WARNING','OFFLINE') ORDER BY HEALTH_SCORE LIMIT 30").to_pandas())
        c1, c2 = st.columns(2)
        c1.metric("Critical/Warning Equipment", len(eq))
        c2.metric("AI Engine", "Cortex Complete")
        if eq.empty:
    ...
    with sc2:
        cat_filter = st.selectbox("Category", ["All", "Machine_Manual", "Technician_Field_Report", "Safety_Bulletin", "Spare_Parts"])
