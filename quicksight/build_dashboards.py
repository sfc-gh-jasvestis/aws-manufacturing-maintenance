...
FROM MANUFACTURING_MAINTENANCE.CURATED.EQUIPMENT_HEALTH""",
    ...
    {
        "id": "maintenance-rul", "name": "Predictive Maintenance: RUL",
        "sql": """SELECT EQUIPMENT_ID, EQUIPMENT_NAME, EQUIPMENT_TYPE,
       STATUS, HEALTH_SCORE::FLOAT AS HEALTH_SCORE,
       PREDICTED_DAYS_TO_FAILURE::INT AS PREDICTED_DAYS_TO_FAILURE,
       FAILURE_RISK, FAILURE_COUNT::INT AS FAILURE_COUNT
FROM MANUFACTURING_MAINTENANCE.ML.RUL_PREDICTIONS""",
        "ds_id": "mfg-maintenance-rul",
        "ds_name": "MFG: Maintenance RUL",
        "kpis": [
            {"label": "Equipment", "field": "EQUIPMENT_ID", "agg": "DISTINCT_COUNT", "type": "string"},
            {"label": "Imminent Risk", "field": "EQUIPMENT_ID", "agg": "DISTINCT_COUNT", "type": "string", "filter": ("FAILURE_RISK", "IMMINENT")},
            {"label": "Avg Days to Failure", "field": "PREDICTED_DAYS_TO_FAILURE", "agg": "AVERAGE", "type": "decimal"},
            {"label": "Avg Health Score", "field": "HEALTH_SCORE", "agg": "AVERAGE", "type": "decimal"},
        ],
        "charts": [
            {"type": "donut", "title": "Failure Risk Distribution",
             "category": ("FAILURE_RISK", "string"), "value": ("EQUIPMENT_ID", "string", "DISTINCT_COUNT")},
            {"type": "bar", "title": "Avg Predicted Days to Failure by Type",
             "x": ("EQUIPMENT_TYPE", "string"), "y": ("PREDICTED_DAYS_TO_FAILURE", "decimal", "AVERAGE"),
             "color": ("FAILURE_RISK", "string")},
        ],
        "q": ["Which equipment is at imminent failure risk?", "What is the avg days to failure?", "How many assets need attention this week?"],
        "topic_id": "mfg-maintenance-rul-q", "topic_name": "Predictive Maintenance RUL",
        "dashboard_id": "mfg-maintenance-rul-dashboard",
        "dashboard_name": "Predictive Maintenance: RUL Predictions",
        "drop_existing": True,
    },
    {
